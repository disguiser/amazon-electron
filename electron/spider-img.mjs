 ...
 * @param {import('puppeteer').Page} page - The Puppeteer page object representing the loaded product page.
 * @returns {Promise<string[]>} A promise that resolves to an array of potential high-resolution image URLs.
 */
export async function getAmazonHighResImgs(page) {
    console.log("Attempting to extract Amazon high-res images (Processing in Node.js)...");
    let imageUrls = new Set(); // Use a Set to automatically handle duplicates
      ...
      for (const scriptContent of rawData.scriptContents) {
        ...
        if (galleryMatch && galleryMatch[1]) {
            try {
                ...
                console.warn("Node.js context: Could not parse 'imageGalleryData'. Falling back to regex.", e.message);
        ...
        if (colorImagesMatch && colorImagesMatch[1]) {
            try {
                ...
                if (colorData && colorData.initial && Array.isArray(colorData.initial)) {
                    colorData.initial.forEach(imgInfo => {
                        if (imgInfo.hiRes) {
                ...
                console.warn("Node.js context: Could not parse 'colorImages' data:", e.message);
            }
            continue; // Move to next script
        }
         ...
         if (jQueryJsonMatch && jQueryJsonMatch[1]) {
             try {
                 ...
                 if (jsonData.imageGalleryData && Array.isArray(jsonData.imageGalleryData)) {
                    jsonData.imageGalleryData.forEach(img => {
                         if (img.mainUrl) imageUrls.add(img.mainUrl);
                         if (img.largeUrl) imageUrls.add(img.largeUrl);
                         if (img.hiResUrl) imageUrls.add(img.hiResUrl);
                     });
                 ...
                 if (!foundInJson && jsonData.colorImages && jsonData.colorImages.initial && Array.isArray(jsonData.colorImages.initial)) {
                     jsonData.colorImages.initial.forEach(imgInfo => {
                 ...
                 console.warn("Node.js context: Could not parse JSON data from jQuery.parseJSON:", e.message);
             }
             continue; // Move to next script
         }
      ...
      if (imageUrls.size === 0) {
          ...
          if (rawData.mainImageDynamicData) {
              try {
                  ...
                  console.warn("Node.js context: Could not parse 'data-a-dynamic-image'. Falling back to src.", e.message);
